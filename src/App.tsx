...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Create realistic simulated order book if the API rate limits or errors
    const generateSimulatedOrderbook = (basePrice: number) => {
      const refPrice = basePrice || lastLivePrice || 60000;
      const localBids: [number, number][] = [];
      const localAsks: [number, number][] = [];
      for (let i = 0; i < 8; i++) {
        const bidPrice = refPrice * (1 - (i + 1) * 0.00018 - Math.random() * 0.0001);
        const askPrice = refPrice * (1 + (i + 1) * 0.00018 + Math.random() * 0.0001);
        localBids.push([bidPrice, Math.random() * 1.5 + 0.005]);
        localAsks.push([askPrice, Math.random() * 1.5 + 0.005]);
      }
      setOrderBook({
        bids: localBids.sort((a, b) => b[0] - a[0]),
        asks: localAsks.sort((a, b) => a[0] - b[0])
      });
    };
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (active && data.success && data.bids && data.asks && data.bids.length > 0) {
            ...
            generateSimulatedOrderbook(targetBasePrice);
          }
        } else if (active) {
          generateSimulatedOrderbook(targetBasePrice);
        }
  ...
  // Seed changes slightly every 1.2s to simulate flashing order book & trades
  useEffect(() => {
    const interval = setInterval(() => {
      setOrderBookSeed(s => s + 1);
    }, 1200);
    return () => clearInterval(interval);
  }, []);
  ...
  // Generate simulated local order book bids and asks
  const activeChartPrice = activeSelectedOrder?.current_price || activeSelectedOrder?.entry_price || 90500;
  
  const simulatedAsks: { price: number; size: number }[] = [];
  const simulatedBids: { price: number; size: number }[] = [];
  for (let i = 1; i <= 5; i++) {
    const askPrice = activeChartPrice * (1 + (i * 0.00012 + (Math.sin(orderBookSeed + i) * 0.00002)));
    const bidPrice = activeChartPrice * (1 - (i * 0.00012 + (Math.cos(orderBookSeed + i) * 0.00002)));
    const askSize = Math.abs(Math.sin(orderBookSeed * 1.5 + i) * 1.8) + 0.15;
    const bidSize = Math.abs(Math.cos(orderBookSeed * 1.6 - i) * 1.9) + 0.15;
    simulatedAsks.push({ price: askPrice, size: askSize });
    simulatedBids.push({ price: bidPrice, size: bidSize });
  }
  simulatedAsks.reverse(); // high prices at top

  // Generate simulated trades
  const simulatedTrades: { isBuy: boolean; price: number; size: number; time: string }[] = [];
  for (let i = 0; i < 5; i++) {
    const isBuy = (orderBookSeed + i) % 2 === 0;
    const priceWithSlip = activeChartPrice * (1 + (Math.sin(orderBookSeed * 0.8 + i) * 0.00008));
    const size = Math.abs(Math.sin(orderBookSeed + i * 2) * 4) + 0.2;
    const time = new Date(Date.now() - i * 3600).toLocaleTimeString("fa-IR", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
    simulatedTrades.push({ isBuy, price: priceWithSlip, size, time });
  }
    ...
    <div className="min-h-screen bg-[#0C0D10] text-[#EAECEF] font-sans overflow-x-hidden selection:bg-[#F0B90B]/20 select-none pb-12" dir="rtl">
      ...
      <div className="bg-[#12161A] border-b border-[#1E2329] py-1.5 overflow-hidden text-[10px] font-mono whitespace-nowrap text-slate-400 flex items-center relative z-40" dir="ltr">
        <div className="flex animate-marquee gap-10">
          <div className="flex items-center gap-8 pr-12 shrink-0">
            ...
            <span className="flex items-center gap-1.5">
              <strong className="text-white">COGNITIVE LATENCY:</strong> <span className="text-[#F0B90B] font-bold">0.86ms (HIGH SPEED)</span>
            </span>
            <span className="text-slate-700">|</span>
            <span className="flex items-center gap-1.5">
              <strong className="text-white">SCAN RATE:</strong> <span className="text-emerald-400">1482 ASSETS/SEC</span>
            </span>
            <span className="text-slate-700">|</span>
            <span className="flex items-center gap-1.5">
              <strong className="text-white">XT NETWORK ENGINE:</strong> <span className="text-[#F0B90B] font-semibold">ACTIVE WEBSOCK v4</span>
            </span>
            <span className="text-slate-700">|</span>
            <span className="flex items-center gap-1.5">
              <Flame size={11} className="text-amber-500 animate-bounce" />
              <strong className="text-white">GARCH RISK FILTER:</strong> <span className="text-purple-400 font-bold">ONLINE</span>
            </span>
            <span className="text-slate-700">|</span>
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              <strong className="text-white">TELEGRAM PIPELINE:</strong> <span className="text-emerald-400">SYNCED OK (Realized 50% Profit messages active)</span>
            </span>
          </div>
      ...
      <header className="bg-[#181A20] border-b border-[#242731] sticky top-0 z-40 transition-colors duration-200">
        <div className="max-w-[1720px] mx-auto px-4 h-16 flex items-center justify-between">
          ...
          {/* XT CENTER TERMINAL BRANDING BADGE */}
          <div className="hidden lg:flex items-center gap-2.5 bg-[#0C0D10]/95 px-4 h-10 rounded-xl border border-[#2B3139] shadow-inner select-none">
            ...
            <span className="font-mono text-[10px] font-black tracking-widest text-[#F0B90B] uppercase">ASHER ENGINE V4.0</span>
            <span className="text-[10px] text-slate-500 font-bold border-r border-[#2B3139] pr-2.5 h-4 flex items-center">اسکنر لایو کورتکس صرافی XT</span>
          </div>
