...
interface CortexDiagnosticsHubProps {
  status: {
    ...
    activeAdaptiveCooldowns?: Record<string, number>;
...
export function CortexDiagnosticsHub({ status, onRefresh }: CortexDiagnosticsHubProps) {
  ...
  const cooldowns = status.activeAdaptiveCooldowns || {};
  ...
  const now = Date.now();
  const activeQuarantines = Object.entries(cooldowns)
    .map(([key, expiry]) => ({ name: key, remaining: expiry - now }))
  ...
  return (
    <section className="bg-[#181A20] border border-cyan-500/20 hover:border-cyan-500/40 transition-colors rounded-xl p-4.5 space-y-4 shadow-lg text-right relative overflow-hidden">
      ...
      {/* QUARANTINED ASSETS STATUS LIST */}
      <div className="bg-black/20 border border-white/[0.03] p-3 rounded-xl space-y-2 relative z-10 text-right">
        <div className="flex items-center justify-between border-b border-white/5 pb-1.5 flex-wrap gap-2">
          <span className="text-[10px] text-slate-300 font-bold flex items-center gap-1">
            ...
            نمادهای قرنطینه و خنک‌سازی شده (Quarantined Assets)
          </span>
          <div className="flex items-center gap-1.5">
            ...
            {activeQuarantines.length > 0 && (
              <button
                ...
                title="آزاد کردن کلیه کوین‌های قرنطینه شده"
              >
                {loadingAction === "all" ? "درحال ریست..." : "ریست کل قرنطینه‌ها"}
              </button>
            )}

            <span className="bg-cyan-500/10 text-cyan-400 font-mono text-[9px] px-1.5 py-0.5 rounded">
              {activeQuarantines.length} کوین فعال
            </span>
          </div>
        </div>
        ...
        {activeQuarantines.length > 0 ? (
          <div className="grid grid-cols-2 gap-2 text-xs">
            {activeQuarantines.map((item, idx) => (
              <div 
                key={idx} 
              ...
              >
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    title={`حذف ${item.name} از قرنطینه`}
                  >
                    <Trash2 size={9} />
                    <span className="text-[8.5px] font-extrabold">آزاد</span>
                  </button>
                ...
                <div className="text-right">
                  ...
                  <span className="text-[8px] text-slate-500 block leading-none">ممنوعیت سیگنال</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-slate-600 text-[10.5px] italic text-center py-3 flex items-center justify-center gap-1.5">
            ...
            هیچ نمادی در قرنطینه نیست. ورود آزاد برای تمام واگرایی‌ها معتبر است.
          </div>
        )}
      </div>
