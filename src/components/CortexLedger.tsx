...
export function CortexLedger({
  ...
  const totalClosed = profitTradesCount + lossTradesCount;
  const winRate = totalClosed > 0 ? (profitTradesCount / totalClosed) * 100 : 0;
  
  const netEarningsUsd = profitTotalUsd + lossTotalUsd;
  const isNetProfit = netEarningsUsd >= 0;

  // Calculate true portfolio Cumulative ROI based on total starting capital
  const currentCapital = status?.tradingMode === "real"
    ? (status?.realBalance || 0)
    : (status?.demoBalance || 1000);
  
  const startingCapital = currentCapital - netEarningsUsd;
  const finalStartingCapital = startingCapital > 0 ? startingCapital : 1000;
  const cumulativePct = (netEarningsUsd / finalStartingCapital) * 100;

  // Find extreme trades
  const allClosed = [...profitParts, ...lossParts].sort((a, b) => b.pnl_usd - a.pnl_usd);
  const bestTrade = allClosed.length > 0 ? allClosed[0] : null;
      ...
      <div className="bg-black/15 p-2.5 rounded-lg border border-[#2B3139]/40 space-y-1.5 text-xs text-slate-400">
        ...
        <div className="flex justify-between items-center border-t border-white/[0.03] pt-1.5">
          ...
          <span className="font-mono text-slate-200 font-bold text-[11px]">
            {(profitTotalUsd / Math.max(1, Math.abs(lossTotalUsd))).toFixed(2)} PF
          </span>
        </div>
      </div>
