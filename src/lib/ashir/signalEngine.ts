...
import { VolRegimeStrategy, LiquidityStrategy, FundingStrategy, CorrelationStrategy, TimeSniperStrategy, AdvancedConfluenceScalper, MicroScalpStrategy } from "./strategies";
...
export class SignalEngine {
  ...
  async analyze(symbol: string, klines: Klines, orderbook: OrderBook | null, change24h = 0, btcChange = 0, livePrice = 0): Promise<Signal | null> {
    ...
    if (action !== "stay_out") {
      // mlConfidence معمولاً در بازه‌ی [minMLConfidence , ~0.95] قرار دارد.
      // آن را به بازه‌ی 0..1 نگاشت می‌کنیم تا تبدیل به اهرم 10..50 شود.
      ...
      // ضریب کاهش ریسک بر اساس نوسان: نوسان بالا => اهرم کمتر، حتی با اعتماد بالا
      const volRiskDampener = Math.max(0.3, Math.min(1.0, 0.012 / relativeATR));

      const rawLeverage = 10 + normalizedConfidence * 40; // 10x .. 50x
